...
// Tambahkan fungsi baru ini di script.js (misalnya, setelah function toast)
async function loadNotifications() {
    ...
    try {
        ...
        // Update UI dan tampilkan di pop-up
        updateNotificationUI(); 
        renderNotificationDropdown(); 
    } catch (err) {
...
function subscribeToNotifications() {
    ...
    // 1. Unsubscribe dari channel lama (jika ada)
    unsubscribeFromNotifications(); // Fungsi ini mungkin perlu Anda buat untuk memanggil sb.removeChannel(notificationChannels)
    ...
    channel.on('postgres_changes', { 
        ...
        schema: 'public', 
        table: 'notifications',
        filter: `user_id=eq.${userId}` // Dengarkan hanya untuk notifikasi user ini
    }, (payload) => {
        console.log('Notifikasi Realtime Diterima!', payload.new);
        showNotificationAlert(payload.new); // payload.new adalah object notifikasi yang baru di-insert
    })
    .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
            console.log('✅ Realtime Notifikasi Connected');
        } else {
            console.error('Notifikasi subscription error', err);
        }
    });

    // Simpan channel agar bisa di-unsubscribe saat logout
    notificationChannels.push(channel);
}

// Catatan: Anda juga perlu memastikan membuat fungsi unsubscribeFromNotifications() 
// yang memanggil sb.removeChannel() pada semua channel di array notificationChannels.
 ...
 */
function renderNotificationDropdown() {
    const listEl = $('#notification-list'); // Pastikan Anda memiliki div dengan id="notification-list" di HTML
    if (!listEl) return;

    listEl.innerHTML = ''; // Bersihkan konten lama

    if (notifications.length === 0) {
        listEl.innerHTML = '<div style="padding:10px; text-align:center;">Tidak ada notifikasi baru.</div>';
        return;
    }
    ...
    notifications.forEach(n => {
        ...
        const timeText = new Date(n.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
        
        item.innerHTML = `
            <div style="font-weight:700;">${escapeHTML(n.title)}</div>
            <div class="small">${escapeHTML(n.message)}</div>
            <div class="small" style="color:var(--muted); margin-top:4px;">${timeText}</div>
        `;
        
        // Contoh: Membuat item bisa diklik untuk menandai sudah dibaca
        item.onclick = () => {
            if (!n.is_read) {
                markNotificationAsReadInDB(n.id);
            }
            // Tambahkan logika lain, misal navigasi
        };
