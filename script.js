...
// Fungsi ini menangani konversi path file Supabase Storage menjadi URL publik
...
function renderProductCard(product) {
  ...
  // Tambahkan class agar styling CSS Anda bekerja
  card.className = 'card product-item'; 
  
  // Panggil fungsi yang ada untuk membuka detail produk
  card.onclick = () => showProductDetail(product.id);

  // Hitung rata-rata rating
  let avgRating = 0;
  if (product.product_reviews && product.product_reviews.length > 0) {
      const totalRating = product.product_reviews.reduce((sum, review) => sum + review.rating, 0);
      avgRating = (totalRating / product.product_reviews.length).toFixed(1);
  }
  ...
  card.innerHTML = `
      <div class="p-2">
          <img src="${escapeHTML(product.file_url)}" 
               alt="${escapeHTML(product.title)}" 
               style="width:100%; height:100px; object-fit:cover; border-radius:8px;">
      </div>
      <div class="p-2">
          <div class="small text-muted">${escapeHTML(product.category)}</div>
          <h4 style="margin:4px 0 6px 0; font-size:14px;">${escapeHTML(product.title)}</h4>
          <div style="font-weight:bold; color:var(--brand);">${formattedPrice}</div>
...
// Ambil percakapan di mana user adalah buyer atau seller
...
if (error) {
  console.error(error);
  container.innerHTML = '<div class="small" style="text-align:center;">Gagal memuat percakapan</div>';
  return;
}

if (!convs || convs.length === 0) {
  container.innerHTML = '<div class="small" style="text-align:center;">Belum ada percakapan</div>';
  return;
}
...
convs.forEach(conv => {
  ...
  const role = isBuyer ? 'Penjual' : 'Pembeli';

  const item = document.createElement('div');
  item.className = 'chat-list-item';
  item.innerHTML = `
    <div style="font-weight:700;">${role} ID: ${otherId}</div>
    <div class="small">${new Date(conv.created_at).toLocaleString('id-ID')}</div>
  `;
  item.onclick = () => {
    otherUserId = otherId;
    currentConversationId = conv.id;
    showPage('chat-detail');
    _fallbackLoadChatMessages(conv.id, otherId);
  };
...
// PASTIKAN VARIABEL GLOBAL BERIKUT SUDAH DIDEFINISIKAN SEBAGAI 'let'
// let currentConversationId = null; 

async function startChatWithUser(otherUserId, otherUserName) {
  if (!currentUser) return toast('Login dulu');
  
  // 1. Validasi Input Kritis (Pencegahan error 'without parameters')
  if (!otherUserId) {
      console.error('startChatWithUser error: otherUserId is missing.');
      return toast('Gagal: ID lawan bicara tidak ditemukan.');
  }
  
  // 2. Persiapan UI
  console.log(`Membuka chat dengan ${otherUserName} (ID: ${otherUserId})`);
  $('#chat-detail-header-name').textContent = escapeHTML(otherUserName);
  $('#chat-detail-avatar').textContent = escapeHTML(otherUserName[0].toUpperCase());
  $('#chat-bubbles-container').innerHTML = '<div class="small" style="text-align:center">Memuat pesan...</div>';
  
  showPage('chat-detail');
  
  try {
      // 3. Panggil RPC Supabase (Menggunakan nama fungsi RPC yang sudah benar)
      const { data: rpcResult, error: rpcError } = await sb.rpc('get_or_create_conversation', {
          // ✅ PARAMETER yang BENAR (mencegah error 'without parameters')
      ...
      // 4. Ekstraksi ID Percakapan (Mencegah error '[object Object]' for UUID)
      if (typeof rpcResult === 'object' && rpcResult !== null) {
          // Asumsi: Mencari di properti 'id' atau 'conversation_id'
          finalConvId = rpcResult.id || rpcResult.conversation_id; 
          
          // Handle jika hasil RPC adalah array [ {id: 'uuid'} ]
          if (!finalConvId && Array.isArray(rpcResult) && rpcResult.length > 0) {
               finalConvId = rpcResult[0].id || rpcResult[0].conversation_id;
          }
      } else {
          // Jika hasil RPC sudah string UUID (scalar)
          finalConvId = rpcResult;
      }

      if (!finalConvId || typeof finalConvId !== 'string') {
           throw new Error("RPC gagal mengembalikan ID percakapan valid (UUID).");
      }

      // 5. Penetapan ke Variabel Global (Mencegah error 'Assignment to constant variable')
      currentConversationId = finalConvId;
      console.log('Conversation ID:', currentConversationId);
      
      // 6. Lanjutkan Proses Chat
      await loadMessages(currentConversationId);
      subscribeToMessages(currentConversationId);
      
  } catch (err) { // ✅ Variabel error yang BENAR
      // ❌ JANGAN gunakan 'error' di sini. Selalu gunakan 'err'.
      console.error('startChatWithUser error:', err);
      
      $('#chat-bubbles-container').innerHTML = `<div class="small text-danger" style="text-align:center">❌ Gagal memulai chat: ${err.message || 'Terjadi kesalahan tidak diketahui.'}</div>`;
      
      toast('❌ Gagal memulai chat: ' + (err.message || 'Terjadi kesalahan.'));
  }
}
...
.on(
  'postgres_changes',
  {
    event: '*',              // bisa 'INSERT' kalau hanya mau pesan baru
    schema: 'public',
    table: 'messages',
...
.subscribe((status, err) => {
  ...
  else console.log('Chat subscription status:', status);
});


        
    
    notificationChannels.push(chatChannel);
...
.on(
  ...
  payload => {
    console.log('Notifikasi Penjualan Baru Diterima!', payload.new);
    addNotification(`Produk Anda terjual! (Order ${payload.new.order_id})`, 'sale');

    if ($('#my-shop').classList.contains('active')) {
      loadMyProducts();
    }
  }
)
...
notificationChannels.push(salesChannel);

console.log('✅ Berlangganan notifikasi realtime...');
}
...
try {
  ...
  const filePath = `${BUCKET_NAME}/${filename}`; // Path yang disimpan di database
  // ------------------------------------------
...
async function loadChatListPage() {
    ...
    try {
        // 1. Ambil percakapan DAN data profil lawan bicara (buyer & seller)
        const { data: convs, error } = await sb
            .from('conversations')
            .select(`
                id, 
        ...
        container.innerHTML = ''; // Bersihkan "Memuat..."
        if (empty) empty.style.display = 'none';

        convs.forEach(conv => {
            // 2. Tentukan siapa "lawan bicara"
            const isMeBuyer = conv.buyer.id === currentUser.id;
            const otherUser = isMeBuyer ? conv.seller : conv.buyer;

            // Handle jika profil user lain tidak ditemukan
            if (!otherUser) {
                console.warn('Profil lawan bicara tidak ditemukan untuk percakapan:', conv.id);
                return; // Lewati percakapan ini
            }

            const otherUserName = otherUser.full_name || 'Pengguna';
            
            // 3. Siapkan Avatar
            // Kita gunakan helper getAvatarUrl dan createFallbackSvg yang sudah ada
            const avatarUrl = getAvatarUrl(otherUser.avatar_url);
            const fallbackSvg = createFallbackSvg(otherUserName, 48, 18); // 48px, font 18px

            // 4. Buat elemen HTML (sesuai kelas CSS di file Anda)
            const item = document.createElement('div');
            item.className = 'chat-list-item';
            item.innerHTML = `
                <div class="avatar">
            ...
            // 5. Perbaiki OnClick: Panggil startChatWithUser (fungsi yang sudah ada)
            // Ini akan membuka halaman chat-detail
            item.onclick = () => {
                startChatWithUser(otherUser.id, otherUserName);
            };
        ...
        container.innerHTML = `<div class="small" style="text-align:center; color: var(--danger);">Gagal memuat percakapan: ${err.message}. Cek RLS.</div>`;
    }
}
...
try {
  ...
  productsContainer.innerHTML = products.map(p => {
    ...
    // Cek apakah produk punya file_url atau image_url
    const filePath = p.file_url || p.image_url;

    if (filePath) {
      if (filePath.startsWith('http')) {
        imgUrl = filePath;
      } else {
        // File dari Supabase Storage bucket "products"
        const { data: img } = sb.storage.from('products').getPublicUrl(filePath);
    ...
    return `
      <div class="product-card card" 
        ...
        <div style="font-weight:600; margin-top:4px;">${escapeHTML(p.name)}</div>
