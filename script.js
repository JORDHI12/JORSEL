...
/**
 * Mendengarkan notifikasi Realtime dari channel Supabase (misal: "notifications:a1b2c3d4...")
 */
async function listenForNotifications() {
    const user = await sb.auth.getUser();
    if (!user.data.user) {
        console.log("Tidak dapat mendengarkan notifikasi, user belum login.");
        return;
    }
    
    const userId = user.data.user.id;
    const channelName = `notifications:${userId}`; // Channel khusus untuk user ini

    // 1. Unsubscribe dari channel lama (jika ada) untuk menghindari duplikasi
    sb.realtime.getChannels()
        .filter(c => c.topic.startsWith('realtime:notifications:'))
        .forEach(c => sb.removeChannel(c));

    // 2. Subscribe ke channel baru
    const notificationChannel = sb.channel(channelName);

    notificationChannel
        .on('postgres_changes', { 
            event: '*', 
            schema: 'public', 
            table: 'pg_notify' // Ini adalah fiktif, kita mendengarkan channel pg_notify
        }, payload => {
            // Logika ini tidak akan terpanggil karena kita menggunakan pg_notify
        })
        .subscribe((status, err) => {
            if (status === 'SUBSCRIBED') {
                console.log(`✅ Realtime Channel '${channelName}' aktif.`);
            }
            if (err) {
                console.error("Gagal subscribe ke Realtime:", err);
            }
        });

    // 3. Mendengarkan pesan dari pg_notify secara langsung (metode yang benar)
    notificationChannel.on('broadcast', { event: 'REALTIME_POSTGRES_CHANGES' }, (payload) => {
        // payload.payload adalah string JSON yang dikirim dari pg_notify
        try {
...
/**
 * Memperbarui badge notifikasi di lonceng.
 */
...
/**
 * Merender daftar notifikasi ke dalam dropdown.
 */
function renderNotificationDropdown() {
    const listEl = document.getElementById('notification-list');
    
    if (notifications.length === 0) {
        listEl.innerHTML = '<p class="small" style="text-align: center; color: #888;">Tidak ada notifikasi baru.</p>';
        return;
    }

    listEl.innerHTML = notifications.map(n => `
        <div style="padding: 10px; border-bottom: 1px solid #eee; background: ${n.read ? 'white' : '#fffbe6'}; cursor: pointer;"
             onclick="showPage('product-detail', '${n.product_id}'); markNotificationRead(${n.id})">
            <strong style="font-size: 13px;">[BARU] ${n.seller_name}</strong>
            <p class="small" style="margin: 2px 0;">${n.message}</p>
            <span style="font-size: 10px; color: #aaa;">${n.timestamp}</span>
        </div>
    `).join('');
}

/**
 * Dipanggil saat notifikasi baru masuk (dari Realtime).
 */
function addNotificationToList(data) {
    ...
    notifications.unshift(newNotification); // Tambahkan ke paling atas
    unreadCount++; 
    updateNotificationUI(); // Update badge lonceng
    
    // Opsional: Tampilkan pop-up singkat di layar (Toast)
    console.log(`Notifikasi baru diterima: ${newNotification.message}`);
}

/**
 * Menandai notifikasi sebagai sudah dibaca (dihubungkan ke tombol "Tandai Sudah Dibaca").
 */
function clearNotifications() {
    ...
    renderNotificationDropdown();
    // Tutup dropdown setelah dibaca
    document.getElementById('notification-dropdown').style.display = 'none'; 
}

// Fungsi pembantu jika mengklik item notifikasi
...
async function loadNotifications() {
    // Pastikan ID pengguna sudah tersedia sebelum memuat data
    if (!currentUser || !currentUser.id) {
        console.warn("⚠️ Pengguna belum login atau ID tidak tersedia.");
        // Anda bisa menampilkan pesan "Harap login" di daftar notifikasi jika perlu.
        return;
    }

    try {
        // [KOREKSI PENTING] HANYA masukkan nama kolom yang dipisahkan koma.
        // Komentar (//...) harus dihapus atau diletakkan di luar string backtick (`...`).
        const { data: notifications, error } = await sb
            ...
            .select(`
                ...
                related_id
            `) 
            .eq('user_id', currentUser.id) // Filter notifikasi untuk user ini
            .order('created_at', { ascending: false }); // Urutkan dari terbaru

        if (error) {
            console.error('❌ Gagal memuat notifikasi (Kemungkinan RLS/DB Error):', error.message);
            // Tampilkan pesan error di UI
            $('#notification-list').innerHTML = `<div class="notification-item text-center error" style="color: red;">Gagal memuat: ${error.message}</div>`;
            return;
        }

        console.log('✅ Notifikasi berhasil dimuat:', notifications);
        
        // Panggil fungsi untuk menampilkan data ke UI
        

    } catch (err) {
        console.error('Error saat memuat notifikasi:', err);
    }
}
// Panggil fungsi ini di dalam postLoginSetup:
async function postLoginSetup(){
  // ... kode lainnya ...
  
  subscribeToNotifications(); 
  loadNotifications(); // 🌟 PERBAIKAN: Memuat daftar notifikasi statis
  
  // ... kode lainnya ...
}
// Ganti seluruh fungsi listenForNotifications yang ada dengan kode yang lebih sederhana ini
function subscribeToNotifications() {
    // 1. Cek User
    if (!currentUser) {
        console.log("⚠️ Tidak bisa subscribe notifikasi: User belum login.");
        return;
    }
    
    const userId = currentUser.id;

    // 2. Bersihkan channel lama agar tidak duplikat
    unsubscribeFromNotifications(); 

    console.log(`🔄 Mencoba subscribe notifikasi untuk user: ${userId}...`);

    // 3. Buat Channel Baru
    const channel = sb.channel(`public:notifications:${userId}`);
    ...
    channel.on('postgres_changes', { 
        ...
        console.log('🔔 Notifikasi Realtime Diterima!', payload.new);
        showNotificationAlert(payload.new); 
    })
    .subscribe((status, err) => {
        // 4. Logging Status yang Lebih Detail
        if (status === 'SUBSCRIBED') {
            console.log('✅ Realtime Notifikasi Terhubung (Connected)');
        } else {
            console.error(`❌ Gagal Subscribe Notifikasi. Status: ${status}`, err || '(Tidak ada detail error)');
            
            // Jika error karena CHANNEL_ERROR, biasanya karena Realtime belum aktif di Dashboard
            if (status === 'CHANNEL_ERROR') {
                console.warn("💡 TIP: Pastikan 'Realtime' sudah diaktifkan untuk tabel 'notifications' di Supabase Dashboard.");
            }
        }
    });

    // Simpan channel ke array global
...
/**
 * Mengisi daftar notifikasi di popup lonceng.
 */
// script.js

// Pastikan fungsi escapeHTML sudah ada (dari script.js Anda)
...
/**
 * Memproses array notifikasi dan memperbarui tampilan UI (daftar & badge).
 * @param {Array} notifications - Array objek notifikasi dari Supabase.
 */
function renderNotifications(notifications) {
    const listElement = $('#notification-list');
    ...
    if (notifications.length === 0) {
        ...
        listElement.innerHTML = '<div class="notification-item text-center small" style="text-align: center; opacity: 0.7;">Tidak ada notifikasi saat ini.</div>';
    }

    // 2. Loop dan Render Notifikasi
    notifications.forEach(n => {
        ...
        const item = document.createElement('div');
        // Gunakan class 'unread' untuk notifikasi yang belum dibaca
        item.className = 'notification-item' + (!n.is_read ? ' unread' : '');
        item.dataset.id = n.id;
        
        const date = new Date(n.created_at).toLocaleString('id-ID', {
            hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short'
        });

        item.innerHTML = `
            <strong>${escapeHTML(n.title)}</strong>
            <p style="margin: 4px 0 0; font-size: 12px; opacity: 0.8;">${escapeHTML(n.message)}</p>
            <span style="font-size: 10px; opacity: 0.6; display: block; margin-top: 4px;">${date}</span>
        `;
        
        // Opsional: Tambahkan event listener untuk menandai notifikasi dibaca saat di-klik
        // item.addEventListener('click', () => { markNotificationAsRead(n.id); });

        listElement.appendChild(item);
    });
    
    // 3. Update Badge Hitungan
    badgeElement.textContent = unreadCount;
    // Tampilkan/sembunyikan badge berdasarkan hitungan
...
// 🔔 Perbaikan: Panggil loadNotifications() dan renderNotificationDropdown() saat lonceng dibuka
$('#btn-wishlist').addEventListener('click', () => { 
    const popup = $('#notif-popup');
    notifVisible = !notifVisible;
    ...
    if (notifVisible) {
        ...
        // 2. Muat dan Render Daftar Notifikasi
        loadNotifications(); // Panggil fungsi yang sudah Anda lengkapi
        
        // 3. Reset Badge UI secara instan (atau nanti di dalam loadNotifications)
        // Jika Anda menggunakan newNotificationCount untuk realtime, di sini tempatnya:
        newNotificationCount = 0;
        $('#notif-badge').style.display = 'none';
        $('#notif-badge').textContent = '0';
        ...
        // Opsi: Panggil fungsi untuk menandai semua notif yang ditampilkan sebagai 'is_read' di DB
        // markAllNotificationsAsRead();
    }
});
// Tambahkan fungsi unsubscribe (Dipanggil saat logout)
function unsubscribeFromNotifications() {
    notificationChannels.forEach(channel => sb.removeChannel(channel));
    notificationChannels = [];
    console.log('✅ Unsubscribed from all notification channels');
}

// Tambahkan fungsi ini (Dipanggil saat notifikasi realtime baru masuk)
function showNotificationAlert(data) {
    // 1. Panggil loadNotifications untuk memuat ulang daftar dari DB (termasuk notif baru)
    loadNotifications(); 

    // 2. Tampilkan notif pop-up singkat (toast)
    const message = data.title + ': ' + data.message;
    toast(message); 
    
    // 3. Update badge (loadNotifications() sudah memanggil updateNotificationUI())
}

// Tambahkan fungsi markAsRead untuk mengupdate status di DB
async function markNotificationAsReadInDB(notificationId) {
    try {
        await sb
            .from('notifications')
            .update({ is_read: true })
        ...
        loadNotifications(); // Muat ulang untuk update UI
    } catch (err) {
        console.error('Mark as read error:', err);
    }
}
// Contoh: Fungsi ini dipanggil setelah pengguna A melakukan aksi
async function createNewNotification(recipientId, notificationTitle, notificationMessage) {
    try {
        const { data, error } = await sb
            .from('notifications') // <-- Pastikan nama tabel sudah benar
            .insert([
                {
                    user_id: recipientId, // ID pengguna yang akan menerima notifikasi
                    title: notificationTitle,
                    message: notificationMessage,
                    is_read: false, // Notifikasi baru selalu belum dibaca
                    // created_at (jika tidak otomatis dari DB)
                }
            ]);

        if (error) {
            console.error('❌ Gagal menyimpan notifikasi:', error.message);
            // Kemungkinan: Error RLS, atau kolom/nilai yang salah
            return;
        }
        
        console.log('✅ Notifikasi berhasil disimpan:', data);
        
        // PENTING: Karena data sudah tersimpan di DB, listener realtime Supabase
        // akan otomatis memicu loadNotifications() di sisi client penerima notif!

    } catch (err) {
        console.error('Error saat insert notifikasi:', err);
    }
}
// script.js

// ... (fungsi loadNotifications() dan renderNotifications() Anda di atas) ...

// Pastikan fungsi utama dipanggil setelah DOM siap
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Content Loaded. Starting notification load...");
    
    // Panggil fungsi utama di sini
    loadNotifications(); 
    
    // Opsional: Panggil setup listener atau inisiasi lainnya
...
function setupNotificationListener() {
    if (!currentUser || !currentUser.id) {
        ...
        console.warn("⚠️ Listener notifikasi tidak diaktifkan: Pengguna belum login.");
        return; 
    }
    
    // PENTING: Gunakan filter pada channel Realtime
    sb.channel('notifications_channel')
        .on('postgres_changes', { 
            event: 'INSERT', 
            schema: 'public', 
            table: 'notifications', 
            // FILTER KRITIS: Hanya dengarkan notifikasi untuk user ini
            filter: `user_id=eq.${currentUser.id}` 
        }, payload => {
            console.log('🔔 Notifikasi Realtime Masuk!', payload.new);
            // Muat ulang daftar notifikasi 
            loadNotifications(); 
        })
        .subscribe();
        
    console.log(`✅ Mencoba subscribe notifikasi untuk User ID: ${currentUser.id}`);
}

// Pastikan ini dipanggil setelah currentUser.id terisi (misal di DOMContentLoaded)
// setupNotificationListener();
// ( ... sisa fungsi Anda ... )
